...
def get_meal_url_for_date(meal_time, target_date):
    return f"https://www.mealtify.com/univ/pusan/student-union/meal/{target_date}/{meal_time}"

def extract_menu_info(item_title):
...
def crawling_and_write_to_csv(url, csv_writer, target_date, meal_time):
    ...
    if response.status_code == 200:
        ...
        for item_title in item_titles:
            menu_string, meal_type = extract_menu_info(item_title)

            # Write the data to the CSV file
            csv_writer.writerow([menu_string, meal_time, target_date, "학생회관", meal_type])
...
def main():
    ...
    csv_file_path = "menu_student_union.csv"
    ...
    print("이번주 월요일부터 금요일까지의 학생회관 메뉴입니다.")
