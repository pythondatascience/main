...
def get_meal_url_for_date(meal_time, target_date):
    return f"https://www.mealtify.com/univ/pusan/saetbeol/meal/{target_date}/{meal_time}"
...
def crawling_and_write_to_csv(url, csv_writer, target_date, meal_time):
    ...
    if meal_time == "lunch":
        ...
        item_titles= [item_title for item_title in item_titles if "일반" in item_title.text]
        for item_title in item_titles:
            menu_string, meal_type = get_menu_info(item_title)

            # Write the data to the CSV file
            csv_writer.writerow([menu_string, meal_time, target_date, "샛별회관", meal_type])
...
def main():
    ...
    with open(csv_file_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for _ in range(5):
            ...
            for meal_time in meal_times:
                if meal_time != "dinner" or (_ != 4):  # Exclude dinner on Friday
    ...
    print("이번주 월요일부터 금요일까지의 샛별회관 메뉴입니다.")
